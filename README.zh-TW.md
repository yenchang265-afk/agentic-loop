...
**多種類迴圈（loop）框架**：每種迴圈類型都是
[`packages/core/loops/<kind>/`](packages/core/loops/README.md) 下的一份宣告式清單
...
目前已發布五種迴圈類型。**engineering**（預設開啟）在 `docs/tasks/` 任務待辦
...
把關點上的任務（永遠不會是草稿）。**`replan [id] [reason]`** 是唯一的拒絕
...
並執行 `/agentic-loop:engineering approve`，它會推送分支並開啟（或重複使用）一個
...
`/agentic-loop:<kind>` 指令，共用 `claim` / `status` / `stop` 動詞（在
OpenCode 上還有 `watch [trigger]` / `unwatch`）。它們都是**實驗性的**——
其清單、設定項和預設值都可能還會變動。依儲存庫在 `.agentic-loop.json` 中
啟用：
...
git clone <this-repo>
cd agentic-loop
npm install             # npm workspaces —— 同時建置 @agentic-loop/core（prepare）
...
- 安裝完成後，互動式終端機會得到一個簡短的**設定精靈**來產生
  `.agentic-loop.json`——見 [docs/configuration.md](docs/configuration.md)。
...
./scripts/clean.sh --purge     # 同時刪除待辦任務檔案 + .agentic-loop.json
```
...
- **`./uninstall.sh`** 會移除本儲存庫連結進你 OpenCode 設定中的
  ...
  `mcp-server/dist`。它不會動你的 `.agentic-loop.json` 和待辦任務；解除安裝
  Claude 外掛本身需要 `/plugin uninstall agentic-loop`。
- **`./scripts/clean.sh`** 清除驅動該專案的迴圈的本機狀態（`$AGENTIC_LOOP_DIR`
  或目前目錄）。預設只清空暫存的 `<tasksDir>/runs/` 機器記憶——快照、指標、
  階段標記、watch 租約、認領標記，以及各種類型的去重帳本——迴圈會重新產生
  這些內容。`--backlog` 還會刪除各狀態資料夾中的任務檔案（保留 `.gitkeep`
  和資料夾本身），`--config` 還會移除 `.agentic-loop.json`，`--purge` 三者
  全做。破壞性等級會先詢問確認（用 `-y` 略過）；`--dry-run` 只預覽不刪除。

## 指令

- `/agentic-loop:engineering new <idea>` · `retask <id> [note]` —— 透過訪談得到一份或多份
  planless 草稿，存於 `docs/tasks/draft/`；`retask` 會重新訪談並就地重塑
  一份草稿
- `/agentic-loop:engineering approve [id]` —— 唯一的按資料夾驅動的把關點：草稿 → 已排入佇列
  （任務把關）、plan-review → 進行中（計畫把關）、in-review → 已完成
  （發布，在你審查分支 diff 之後）。省略 id 的 `approve` 會推進目前唯一
  停在迴圈等待點上的任務——永遠不會是草稿
- `/agentic-loop:engineering replan [id] [reason]` —— 拒絕動詞：把一份暫存的計畫（或以 id
...
- `/agentic-loop:review-sitter` · `/agentic-loop:dep-sitter` ·
  `/agentic-loop:main-sitter` —— 同樣的 `claim` / `watch`（OpenCode）/
  `unwatch` / `stop` / `status` 動詞，各自作用於自己的類型（透過
  `loops.<kind>.enabled` 按需啟用）
...
- [docs/README.md](docs/README.md) —— `docs/` 下每份文件的索引，以及針對
  某個主題哪份文件是權威版本
- [docs/loops/](docs/loops/README.md) —— 每種類型一份檔案（engineering、
...
- [docs/sitters.md](docs/sitters.md) —— 四個實驗性 sitter 的共同點，
  並索引到 `docs/loops/` 下它們各自的檔案
- [packages/core/loops/README.md](packages/core/loops/README.md) —— 如何編寫一種新的迴圈類型
...
- [docs/configuration.md](docs/configuration.md) —— `.agentic-loop.json`
  參考（使用者層級 + 儲存庫層級分層）、各類型的 `loops` 區塊，以及可選的強化項
  （worktree、審查視角、去識別化）
- [docs/templates/AGENTS.md](docs/templates/AGENTS.md) —— 可複製到由
  agentic-loop 驅動的專案中的起始 `AGENTS.md`/`CLAUDE.md`（迴圈工作流程 +
  skill 對應）
...
- [docs/design/](docs/design/) —— 威脅模型、強化設計紀錄
  （包括 [07 — 多迴圈排程器](docs/design/improvements/07-multi-loop-scheduler.md)）
- [packages/hub/README.md](packages/hub/README.md) —— **管理面板（admin
  hub，測試版）**（`npm run hub -- --dir /path/to/repo` → http://127.0.0.1:4317）：
  迴圈監視器（待辦看板、即時把關點通知、執行歷史、按階段的 token 用量）和
  視覺化迴圈建立器；可以監看一個或多個儲存庫（`--dir` 可重複且支援 `*`
  萬用字元，或者在使用者層級 `~/.agentic-loop.json` 中設定 `hub.repos` —— 不設定
  儲存庫就不會監看）
...
- `packages/core/` —— `@agentic-loop/core`：純粹的迴圈引擎、清單層、
  工作來源 + 排程器、任務儲存、git 隔離、快照、裁定、指標、設定 ——
  兩個外掛共用的一切
- `packages/core/loops/` —— 宣告式的迴圈類型，每種類型一個目錄
  （`engineering/`、`pr-sitter/`、`review-sitter/`、`dep-sitter/`、
  `main-sitter/`）：每種類型一份 `loop.json` 清單 + `stages/*.md` 提示詞範本
- `packages/hub/` —— **管理面板（測試版）**：帶有迴圈監視器和視覺化迴圈
  建立器的本機 web 應用程式（[packages/hub/README.md](packages/hub/README.md)）
...
- `skills/`、`references/` —— 階段 agent 和臨時請求所使用的工作流程庫
  （兩個外掛共用）
- `docs/tasks/` —— `/agentic-loop:engineering` 各動詞讀取的檔案系統任務待辦
