...
它回答一個問題：除了管理面板隨附發布的唯讀監視器和迴圈建立器之外，
...
- **Authority（授權層級）**——這個功能讓瀏覽器上的一次點擊能做到什麼，對照
  ...
  [`proposed-loops.md`](./proposed-loops.md) 相同的分級階梯，外加一個新層級。
- **Cost（成本）**——S / M / L：
...
3. **config-write（設定寫入）**——寫入 `.agentic-loop.json`。**新層級，管理面板
...
| # | 功能 | 補上的落差 | 授權層級 | 成本 | 狀態 |
|---|---------|---------------|-----------|------|--------|
| [1](#1--把關動作) | 把關動作 | `loop/gate.ts`——**沒有任何管理面板呼叫者** | backlog-write, push | M | **已發布** |
...
| [4](#4--設定編輯器) | 設定編輯器 | **完全沒有任何地方會寫入 `.agentic-loop.json`** | config-write | L | **已發布** |
...
活動、執行歷史、token 用量、迴圈建立器。全部都是唯讀，這是刻意的設計——
...
| 核心能力 | 狀態 | 管理面板目前的狀況 |
|---|---|---|
| `loop/gate.ts`——`approveTask:101`、`approvePlan:150`、`replanTask:200`、`shipTask:241` | 已發布、已測試 | **零呼叫者。**管理面板能偵測到把關點（SSE 的 `gate` 事件、`gateStatuses` 欄位高亮），卻對它們一個都無法動作。`packages/hub/README.md:111` 明確表示這部分留待日後處理。 |
...
| `.agentic-loop.json` | —— | **沒有任何東西會寫入它。**`routes/kinds.ts:108` 讓建立器流程以叫你手動編輯檔案作結。 |
| `manifest/template.ts` 的 `renderPrompt:61` | 已發布、已測試 | 未使用。建立器盲目地寫出提示詞草稿。 |

**這個立場在實務上其實已經被打破**：建立器會透過 `POST /api/kinds`
（`routes/kinds.ts:113`）寫入 `loops/<kind>/`。所以誠實的做法是把這道界線
正式定義清楚，而不是假裝它還站得住腳。
...
| 逐類型旋鈕驗證 | —— | **管理面板，僅供參考。**收緊核心是一次破壞性變更。見 [關鍵問題 C](#關鍵問題-c--loops-是-looseobject)。 |
...
"@agentic-loop/core/loop/gate"`。採用 `shared/api.ts:7-8` 的純型別
...
`mergeConfigLayers:248` 會在解析*之前*，把使用者層（`~/.agentic-loop.json`）
合併到儲存庫層**之下**。如果編輯器顯示*生效後*的合併設定，並把它存回儲存
庫的檔案，就會**把使用者層攤平寫進儲存庫的檔案**——把 `ado.pat` 從
`~/.agentic-loop.json` 寫進一個 `config.ts:121-126` 明確警告過必須保持
gitignore 的檔案。
...
4. **Gitignore 防護。**在對**儲存庫**層執行會*設定* `ado.pat` 的寫入
   之前，先執行 `git check-ignore -q .agentic-loop.json`。沒有被忽略
   → **400**，並附上 `config.ts:121-126` 裡的警告文字。兩行程式碼，
   就把一段沒人會讀的文件註解，變成一道在真正關鍵的那一刻會被強制執行
   的防護欄。

### 關鍵問題 C — `loops` 是 `looseObject`
...
**收緊核心 `loops` schema 的做法是錯的**，這也是最需要據理力爭的一個
...
`lintLoopKnobs(rawLoops, boards) → ConfigWarning[]`，共四類，全部都是
...
讀取 `.agentic-loop.json`，繞過了核心的 `loadConfig`，純粹只是為了檢查
`loops.<kind>.enabled`——然後在 :108 產生一個「請手動編輯檔案」的檢查清單
...
`.agentic-loop.json`。**兩個半邊都是必要的**——光有寫入路由，在任何一次
...
- **[`configuration.md`](../configuration.md)**——還沒完成，牽涉到設定
  ...
  旋鈕 lint。**交叉連結 `loops.<kind>` 的旋鈕表**（[關鍵問題
  C](#關鍵問題-c--loops-是-looseobject)），並修正 :126 那句「由該類型自行
