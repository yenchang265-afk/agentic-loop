...
- **工作來源（work source）**——這個類型是重用 `backlog` / `pull-request`，
...
| [review-sitter](#review-sitter) **（已出貨）** | 協作 | `pull-request`（+ `review-requested` 觸發器） | comment | S/M |
...
原始草案：pr-sitter 的鏡像，重用 `pull-request` 加上 `review-requested`
