...
const resolveBase = async () => baseDir ? ((await currentBranch(sh, baseDir)) ?? undefined) : undefined;
...
let active = null;
let activeClaim = null; // the scheduler claim behind `active`, when loop_claim made it
let pending = null; // verdict(s) recorded for the current check stage
let samples = []; // per-run metrics
...
const loadCfg = async () => {
    try {
        ...
        await log("warn", `using default config: ${err.message}`);
...
const writeStageMarker = (stage) => {
    ...
    try {
        ...
        if (stage === null) {
            ...
            fs.writeFileSync(stageMarkerPath(), JSON.stringify({
...
const isOverdue = (deadline, now) => deadline !== null && now > deadline;
...
const claimWarnings = async () => {
    ...
    if (owner && !isLeaseStale(owner, new Date(), staleThresholdMs(owner.intervalMs))) {
        warnings.push(`a live watcher (pid ${owner.pid} on ${owner.host}) holds this clone's watch lease — ` +
    ...
    if (hasAnomalies(anomalies))
        warnings.push(...formatAnomalies(anomalies, config.tasksDir).map((l) => `${l} (loop_doctor repairs)`));
    return warnings;
};
/** The fire payload loop_start/loop_claim return for a fresh claim. */
const firePayload = (state, id) => ({
...
server.registerTool("loop_compose", {
...
server.registerTool("loop_verdict", {
    description: "Record the VERIFY or REVIEW verdict for the running loop. THE ONLY TRUSTED verdict channel — a PASS/FAIL in prose is ignored. Called by the loop-verify/loop-review subagent exactly once per pass. Multiple calls in one stage (multi-lens review) are combined worst-wins.",
    inputSchema: {
    ...
    if (!active)
        return fail("No active loop — verdict ignored.");
    ...
    const rec = { verdict, ...(reason ? { reason } : {}), ...(criteria ? { criteria: criteria } : {}) };
    if (!pending)
        pending = rec;
    else {
...
server.registerTool("loop_isolate", {
    description: "Explicitly ensure the feature/<id> branch (or worktree when worktreesDir is set) exists. Normally loop_start does this; use it standalone only when recovering.",
    inputSchema: {},
}, async () => {
    if (!active)
        return fail("No active loop.");
...
server.registerTool("loop_stage", {
    description: "Set the current stage marker so the PreToolUse hook enforces the right bash allowlist (default-deny for verify/review) and the stage deadline. Call right before spawning EACH stage subagent, plan and build included. Setting 'build' appends the audited 'BUILD started' note the claimability predicates key on.",
    inputSchema: { stage: z.string().min(1) },
}, async ({ stage }) => {
    if (!active)
        return fail("No active loop.");
...
server.registerTool("loop_advance", {
    ...
    await runTerminal(action);
    return ok({
        action: { kind: action.kind, message: action.message },
...
const runTerminal = async (action) => {
    if (!active || (action.kind !== "done" && action.kind !== "stop"))
        return;
    const actor = await gitActor(sh, directory);
    ...
    if (active.task) {
        if (action.kind === "done") {
            ...
            if (cur) {
                ...
                await moveTask(sh, cur, (action.toStatus ?? "in-review"));
            }
            else {
            ...
            await appendNote(sh, active.task, auditNote(action.message, new Date(), actor), log);
            ...
            if (active.stage === "plan")
                await releaseClaim(sh, active.task);
        }
    ...
    if (active.task)
        await clearState(sh, directory, config.tasksDir, active.task.id);
    writeStageMarker(null);
    if (activeClaim) {
        const outcome = { kind: action.kind === "done" ? "done" : "stop", message: detail };
        await activeClaim.source.onTerminal?.(activeClaim.item, outcome);
        activeClaim = null;
    }
    active = null;
};
server.registerTool("loop_checkpoint", { description: "Commit the current build state as a checkpoint on the loop branch/worktree.", inputSchema: { message: z.string() } }, async ({ message }) => {
    if (!active?.git)
        return ok({ committed: false, note: "no isolation active" });
    const done = await commitAll(sh, workTree(), message);
    return ok({ committed: done });
});
server.registerTool("loop_note", { description: "Append a timestamped, secret-redacted audit note to the active loop's task file.", inputSchema: { text: z.string() } }, async ({ text }) => {
    if (!active?.task)
        return fail("No active task-backed loop.");
...
server.registerTool("loop_status", {
    description: "Report the active loop (stage/iteration) plus a whole-backlog roll-up: counts per folder, the actionable flags, and the loop kinds (enabled/disabled).",
    inputSchema: {},
}, async () => {
    await loadCfg();
    const byStatus = {};
    for (const s of STATUSES)
        byStatus[s] = await listByStatus(fsClient, directory, config.tasksDir, s, log);
...
server.registerTool("loop_doctor", {
    description: "Audit the backlog for structural damage a confused agent can cause: stray folders (not a status folder), task files outside every status folder, duplicate ids across status folders, and held claim markers. With fix:true, performs only the unambiguous repairs — rescue stray .md files back to draft/ (audited note + commit), remove now-empty stray folders, and release stale orphaned claim markers. Duplicates are always flagged for a human, never auto-resolved.",
    ...
    if (!fix)
        return ok({ ...report, next: hasAnomalies(anomalies) || Object.keys(heldClaims).length ? "loop_doctor with fix:true applies the unambiguous repairs" : "backlog is clean" });
    const actor = await gitActor(sh, directory);
    const rescued = [];
    const failed = [];
    ...
    for (const status of ["queued", "in-progress"]) {
        ...
        if (released.length)
            releasedClaims[status] = released;
    }
    if (rescued.length) {
...
const approvePlan = async (id) => {
    ...
    if (!hasPlan(task))
        return { ok: false, message: `Task "${id}" has no Implementation Plan — send it back with loop_replan.` };
    const actor = await gitActor(sh, directory);
...
const resolveGateTask = async (id, folders) => {
    ...
    const found = [];
    for (const from of folders) {
        for (const t of await listByStatus(fsClient, directory, config.tasksDir, from, log))
            found.push({ id: t.id, from });
    }
    if (found.length === 0)
...
const approveAny = async (id) => {
    ...
    if (pick.from === "plan-review")
        return approvePlan(pick.id);
    return shipTask(pick.id); // in-review
};
/** ship shortcut: id optional — ships the single in-review/ task when omitted. */
const shipAny = async (id) => {
    if (id)
        return shipTask(id);
    const pick = await resolveGateTask("", ["in-review"]);
    if (!pick.ok)
        return { ok: false, message: pick.kind === "none" ? "Nothing awaiting ship." : pick.message };
 ...
 */
const rejectAny = async (arg, liveTaskId) => {
    const [first = "", ...restParts] = arg.trim().split(/\s+/).filter(Boolean);
    if (first) {
        for (const from of ["plan-review", "in-progress"]) {
    ...
    if (!pick.ok)
        return { ok: false, message: pick.kind === "none" ? "No plan awaiting rejection." : pick.message };
...
server.registerTool("loop_replan", {
    description: "Deterministic /agentic-loop:engineering replan <id> [reason] — the sole rejection verb: reject a parked plan (plan-review/) or send a cap-tripped in-progress/ task back to queued/ with an audited note, so the next PLAN pass addresses why the old plan failed. Refuses tasks a live loop is driving.",
...
server.registerTool("loop_move", { description: "Move a task file to another status folder.", inputSchema: { id: z.string(), status: z.enum(["draft", "queued", "plan-review", "in-progress", "in-review", "completed", "abandoned"]) } }, async ({ id, status }) => {
    await loadCfg();
    const found = await findAnyStatus(id);
    if (!found)
        return fail(`No task "${id}".`);
...
server.registerTool("loop_recover", {
    description: "Resume an interrupted in-progress task from its state snapshot (exact stage) or, failing that, from its persisted plan at BUILD. Refuses never-started tasks (use loop_start/loop_claim) and planless ones (re-plan first). Returns the next action + prompt.",
    ...
    const t = await findByIdIn(sh, directory, config.tasksDir, "in-progress", id);
    if (!t)
        return fail(`No in-progress task "${id}".`);
    if (isClaimable(t))
        return fail(`Task "${id}" never started — start it with loop_start or loop_claim.`);
    if (!isRecoverable(t))
        return fail(`Task "${id}" has no Implementation Plan — send it back to planning with loop_replan.`);
    ...
    if (snap && snap.task?.id === id) {
        ...
        await appendNote(sh, active.task, auditNote(`Recovered from snapshot at ${active.stage}`, new Date(), actor), log);
    ...
    await appendNote(sh, active.task, auditNote("Recovered from persisted plan — re-entering at BUILD", new Date(), actor), log);
    await snapshot();
...
async function runGate(argv) {
    ...
    if (verb === "approve-any")
        result = await approveAny(remainder);
    else if (verb === "reject-any")
        result = await rejectAny(remainder, readStageTaskId());
    else {
        ...
        if (verb === "approve")
            result = await approveTask(id);
        else if (verb === "approve-plan")
            result = await approvePlan(id);
        else if (verb === "replan")
            result = await replanTask(id, reason, readStageTaskId());
        else
            result = { ok: false, message: `Unknown gate verb "${verb}" — expected approve-any, reject-any, approve, approve-plan, or replan.` };
    }
    emit(result);
...
if (process.argv[2] === "gate") {
    runGate(process.argv.slice(3))
        ...
        process.stdout.write(`${JSON.stringify({ ok: false, message: `gate failed: ${err.message}` })}\n`);
        process.exit(1);
    });
}
else {
    main().catch((err) => {
        process.stderr.write(`agentic-loop MCP fatal: ${err.message}\n`);
        process.exit(1);
    });
}
