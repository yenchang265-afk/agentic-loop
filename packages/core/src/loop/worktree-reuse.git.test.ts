...
const noopLog: Log = () => {}
const config = { ...DEFAULT_CONFIG, worktreesDir: ".loop-worktrees" }
const wtOf = (repo: string): string => path.join(repo, ".loop-worktrees", "t1")
...
const seedRepo = async (): Promise<{ repo: string; taskPath: string }> => {
  const repo = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "agentic-loop-worktree-")))
