...
test("worktree paths containing spaces are quoted in the rewrite", () => {
  const spaced = "/mnt/c/Claude Code/repo/.loop-worktrees/t1"
...
test("block reasons name the worktree so the agent can self-correct", () => {
  assert.match(blocked("cd /repo && npm test"), /isolated to its worktree \/repo\/\.loop-worktrees\/my-task/)
...
test("a main-tree absolute path is remapped onto its worktree mirror", () => {
  ...
  assert.deepEqual(pin(`${DIR}/packages/core/src/loop/git.ts`), {
    action: "rewrite",
    value: `${WT}/packages/core/src/loop/git.ts`,
  })
})
