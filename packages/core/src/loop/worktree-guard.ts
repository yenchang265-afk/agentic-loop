...
const escapeReason = (segment: string, worktree: string, pinnedDir: string | null): string | null => {
  ...
  if (gitDir && !isReadOnlySegment(segment)) {
    ...
    if (!underWorktree(worktree, resolved)) {
      return (
        `agentic-loop: this loop is isolated to its worktree ${worktree} — "${segment.trim()}" mutates the tree at ` +
  ...
  if (target) {
    return (
      `agentic-loop: this loop is isolated to its worktree ${worktree} — "${segment.trim()}" reaches ${target}, ` +
...
const walk = (command: string, worktree: string, initialPin: string | null): WalkResult => {
  ...
  for (const segment of splitSegments(command)) {
    ...
    if (cdMatch) {
      ...
      const escapeMsg =
        `agentic-loop: this loop is isolated to its worktree ${worktree} — "${segment.trim()}" leaves it, so the rest of ` +
 ...
 * INSIDE the main tree (`<directory>/.loop-worktrees/<id>`), so every worktree
...
export const pinEditPath = (filePath: string, worktree: string, directory: string, tasksDir: string): PinVerdict => {
  ...
  if (filePath.startsWith("~")) {
    return blockPin(
      `agentic-loop: "${filePath}" starts with \`~\`, which file tools do not expand — pass a real absolute path under the worktree ${worktree}.`,
    )
  }
  ...
  const refuseTasks = blockPin(
    `agentic-loop: task files are driver-owned and live on the main tree — the loop records notes and moves itself; ` +
  ...
  return blockPin(
    `agentic-loop: this loop is isolated to its worktree ${worktree} — ${filePath} is outside both it and the repo ` +
