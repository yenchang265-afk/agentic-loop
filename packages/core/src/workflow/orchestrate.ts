 ...
 * OpenCode plugin (`plugins/opencode/src/workflow/driver.ts`) and the Claude Code
 ...
 */
export const makeManifestCache = (workflowsDir: string, eager: readonly string[] = []): ((kind: string) => LoadedManifest) => {
  const manifests = new Map<string, LoadedManifest>()
  for (const kind of eager) manifests.set(kind, loadManifest(workflowsDir, kind))
  return (kind) => {
    let loaded = manifests.get(kind)
    if (!loaded) {
      loaded = loadManifest(workflowsDir, kind)
  ...
  enabledWorkflowKinds(config)
    ...
    .flatMap((kind): WorkSource[] => {
      ...
      try {
        ...
        void deps.log(
          "warn",
          `workflow kind "${kind}" is enabled in config but its workflows/${kind}/ manifest could not be loaded — skipping it. ${(err as Error).message}`,
        )
        return []
      }
      const base = {
