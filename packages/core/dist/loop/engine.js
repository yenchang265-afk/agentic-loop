...
import { renderPrompt } from "../manifest/template.js";
...
const withoutArtifacts = (state, stages) => {
    if (stages.length === 0)
        return state;
 ...
 */
export const promptContext = (state) => {
    ...
    return {
        ...
        worktree: wt
            ? {
                path: wt,
                instructions: `Worktree: this loop's isolated checkout is ${wt} — every file you read, edit, or ` +
...
export const composePrompt = (loaded, state, target) => {
    const def = stageDef(loaded.manifest, target);
    const tpl = loaded.prompts[def.name];
    if (tpl === undefined)
        throw new Error(`loop kind "${loaded.manifest.kind}" has no prompt loaded for stage "${def.name}"`);
...
const fireAt = (loaded, state, target) => {
...
export const firstStep = (loaded, state) => ({
    state,
        ...
        case "fire": {
            if (effect.countIteration) {
                ...
                if (s.iteration + 1 >= cap) {
                    const message = (effect.capMessage ?? `✗ Loop stopped after {maxIterations} iterations.`).replaceAll("{maxIterations}", String(cap));
