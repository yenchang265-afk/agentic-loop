...
export const metricsPath = (directory, tasksDir, id) => path.join(directory, tasksDir, "runs", `${id}.metrics.json`);
/** Parse a sidecar's raw JSON; null on unparseable or schema-invalid content (fail closed). Pure. */
export const parseRunMetrics = (raw) => {
    let json;
    try {
        json = JSON.parse(raw);
    }
 ...
 */
export const appendRunMetrics = (existingRaw, run) => {
